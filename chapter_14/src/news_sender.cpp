...
int main(int argc, char* argv[])
{
	if (argc != 3)
	{
		...
		print_console(PRINT_INFOR, "Usage: %s <group IP> <port>", argv[0]);
		return -1;
	}
	...
	inet_pton(AF_INET, argv[1], &server_addr.sin_addr.s_addr); // 组播IP
	server_addr.sin_port = htons(atoi(argv[2])); // 组播端口

	int time_live = 64; // TTL
	int ret = setsockopt(client_sock, IPPROTO_IP,	IP_MULTICAST_TTL, (char*)&time_live, sizeof(time_live));
	if (ret == sock_error)
	{
		print_console(PRINT_ERROR, "failed to set multi.");
		return -1;
	}
