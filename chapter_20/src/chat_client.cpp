...
return_type send_msg(void* arg)   // send thread main
{
...
return_type recv_msg(void* arg)   // read thread main
{
	sock_t sock = *((sock_t*)arg);
	char name_msg[NAME_SIZE + BUFFER_SIZE];
	while (true)
...
#if defined(_WIN32) || defined(_WINDLL)
	DWinThreadEx send_thread(send_msg, (d_handle)&client_sock);
	DWinThreadEx recv_thread(recv_msg, (d_handle)&client_sock);
#else
