...
int main(int argc, char* argv[])
{
	...
	int res_send = getsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char*)&send_buf_size, &send_size_len);
	if (res_send)
	{
	...
	int res_recv = getsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char*)&recv_buf_size, &recv_size_len);
	if (res_recv)
	{
		print_console(PRINT_ERROR, "getsockopt() failed");
	}
