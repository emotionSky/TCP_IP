...
#if defined(_WIN32) || defined(_WINDLL)
	const char* str1 = "FROM SERVER: Hi~ client? \n";
	const char* str2 = "I love all of the world \n";
	const char* str3 = "You are awesome! \n";
	send(client_sock, str1, static_cast<int>(strlen(str1)), 0);
	send(client_sock, str2, static_cast<int>(strlen(str2)), 0);
	send(client_sock, str3, static_cast<int>(strlen(str3)), 0);
	shutdown(client_sock, SD_SEND);
	int recv_len = recv(client_sock, message, BUFFER_SIZE - 1, 0);
	if (recv_len > 0)
	{
		message[recv_len] = 0;
